...
var DataSheet = function (_PureComponent) {
  ...
  _createClass(DataSheet, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      if (this.hasHeader) {
        var widths = this.updateCellWidthsFromProps(nextProps.headerData, nextProps.data);
    ...
    key: 'updateCellWidthsFromProps',
    value: function updateCellWidthsFromProps(headerData, bodyData) {
       ...
       */
      var updateWidths = function updateWidths(widths, prevPropsWidthsMap, newData) {
        ...
        newData.forEach(function (row, i) {
          ...
          row.forEach(function (cell, j) {
            ...
            if (cell.width && cell.width != prevPropsWidthsMap.get(i + '.' + j) && cell.width != widths[i][j]) {
              hasChanged = true;
       ...
       */
      var update = function update(has, had, widths, data, prevPropsWidthsMap) {
        if (has) {
      ...
      var buildPrevPropsWidthsMap = function buildPrevPropsWidthsMap(prevPropsData) {
        var map = new Map();

        prevPropsData.forEach(function (row, i) {
          row.forEach(function (cell, j) {
            if (cell.width) {
      ...
      var upHeaderResult = update(this.hasHeader, hadHeader, cellWidths.header, headerData, buildPrevPropsWidthsMap(this.props.headerData));
      var upBodyResult = update(hasBody, hadBody, cellWidths.body, bodyData, buildPrevPropsWidthsMap(this.props.data));
    ...
    value: function calculateCellWidths(cellWidths) {
      var updatedCellWidths = Object.assign({}, this.sizes.cellWidths);
      var headerMainRow = updatedCellWidths.header[updatedCellWidths.header.length - 1];
      var bodyMainRow = updatedCellWidths.body[0];
      ...
      updatedCellWidths.body = updatedCellWidths.body.map(function (row) {
        return mainRow;
      });
